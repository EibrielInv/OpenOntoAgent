...
export class Concept {
    ...
    addConcept(name: string, concept: ConceptDB) :number {
        const concept_id = this.ontoagent.datascript.dbAdd(null, "concept/name", name)
        Object.keys(concept).forEach((k) => {
            if (k == "IS-A") {
                const is_a_id = this.getConceptIdOrAdd(concept[k] as string)
                this.ontoagent.datascript.dbAdd(concept_id, "concept/is-a", is_a_id)
            } else {
                // Is a property
    ...
    conceptExists(concept_name: string) :boolean {
        ...
        const r = ds.q(query, this.ontoagent.datascript.db, concept_name)
        return r.length > 0
    }
    ...
    addConceptProperty(property_name: string, property: any, concept_id: number) {
        const property_id = this.ontoagent.datascript.dbAdd(null, "concept.property/name", property_name)
        this.ontoagent.datascript.dbAdd(concept_id, "concept/property", property_id)
        if (property["SEM"]) {
    ...
    linkProperty(property_id: number, property_name: string, concept:string|Array<string>) {
        if (typeof concept === 'string') {
            ...
            this.ontoagent.datascript.dbAdd(property_id, property_name, sem_concept)
        } else {
            concept.forEach((e)=>{
